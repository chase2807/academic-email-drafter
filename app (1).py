...
try:
    ...
    if not GOOGLE_API_KEY:
        st.error("⚠️ Google API Key not found in Colab secrets. Please add it.")
    else:
...
def generate_email_draft(prompt, intent, recipient_name="", paper_details="", other_info=""):
    ...
    """
    if not GOOGLE_API_KEY:
        return "Error: Gemini API not configured. Please add your API key to Colab secrets."
...
**Note:** Ensure your Google API Key is added to Colab's secrets manager under the name `GOOGLE_API_KEY`.
""")
