...
try:
    # Change this line to read from environment variables
    GOOGLE_API_KEY = os.environ.get('GOOGLE_API_KEY')
    if not GOOGLE_API_KEY:
        # Update the error message to reflect environment variables
        st.error("⚠️ Google API Key not found in environment variables. Please set it.")
    else:
...
def generate_email_draft(prompt, intent, recipient_name="", paper_details="", other_info=""):
    """
    Generates a draft email based on intent and user inputs using the Gemini API.
    """
    # Update this check to use the GOOGLE_API_KEY directly
    if not GOOGLE_API_KEY:
        return "Error: Gemini API not configured. Please set your API key as an environment variable."
    ...
    try:
        ...
        # Catch specific API errors if possible for better debugging, but general Exception is okay for now.
        return f"Error generating email draft with Gemini API: {e}"
...
st.markdown("""
Welcome to the Academic Email Drafter AI assistant. This tool helps you craft professional emails for various academic purposes using the power of Google Gemini.
Please provide a core prompt, select the email's intent, and fill in any optional details to generate a draft.

**Note:** Ensure your Google API Key is set as an environment variable named `GOOGLE_API_KEY` on your deployment platform.
""")
...
if generate_button:
    if not user_prompt:
        ...
        with st.spinner("🤖 Drafting your email..."):
            # Ensure API key is available before attempting generation
            if GOOGLE_API_KEY:
                ...
                st.session_state.draft = draft
                 ...
                 st.session_state.error_message = "⚠️ Gemini API Key not set. Please configure it on your deployment platform."
